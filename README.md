...
Все размеры знака собираются из одного файла — иконка запуска в пяти
плотностях, монохромный слой для тематических иконок, знак в интерфейсе (мелкий
растр под 24 dp и крупный под 120 dp для онбординга), картинка 512×512 для Play
и цвет подложки:

```bash
./scripts/generate-logo.sh                   # из app/src/Yoga-Timing-Assistant.jpg
./scripts/generate-logo.sh path/to/logo.png  # из другого исходника
```

Руками эти файлы не правятся: следующий прогон их перезапишет.
...
| `:feature:stats` | статистика занятий: период, сводка, график, календарь месяца, журнал и его выгрузка в CSV |
...
| [`docs/10-SESSION-TIME.md`](docs/10-SESSION-TIME.md) | время занятия: целевой бюджет, отсечка, режимы паузы, двусторонние асаны |
