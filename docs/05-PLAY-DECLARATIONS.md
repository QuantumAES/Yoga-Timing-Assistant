...
### Состояние реализации (2026-07-28)

Пункты 1–3 выполнены на рабочем экране: подсказка про энергосбережение
показывается один раз (флаг в `HintStore`), не красная, закрывается кнопкой
«Понятно» и рядом даёт переход в `ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS`
(`RestrictionSettings`). Прямой `ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`
не используется.

Важное наблюдение с устройства, которое стоит держать в тексте подсказки:
`PowerManager.isIgnoringBatteryOptimizations` читает **только** системный белый
список Doze. Фирменные энергосберегайки оболочек (MIUI «Нет ограничений»,
Samsung «Не ограничено») ведут в собственные списки вендора и на этот флаг не
влияют — пользователь может выключить у себя всё, что видит, и не понять,
почему приложение продолжает считать себя ограниченным.

Пункт 4 (диагностика по `DriftDetected`) ждёт экрана настроек — Фаза 7.
