...
## 13. Отличия реализации от этого документа

Дописано по итогам Фазы 3 (2026-07-28). Документ утверждался до кода, и в
нескольких местах код оказался правее. Каждое расхождение здесь — осознанное;
незафиксированное расхождение через полгода читается как баг.

### 13.1. Границы модулей: `StageKind` вместо `StageType`

Правило зависимостей направлено сверху вниз (§1): `:domain` знает про движок,
движок про `:domain` — нет. Поэтому в движке живёт собственное перечисление
`StageKind`, а `:domain` переводит в него `StageType` там же, где уже
разрешает наследование оповещений. Цена — четыре значения дубликата; выигрыш —
ядро остаётся без единой внешней зависимости.

По той же причине оповещение приходит в движок **непрозрачной нагрузкой**
(`AlertPayload`): движок отвечает за «когда», а каналы, звуки и фразы меняются
в домене, не задевая ядро отсчёта. Выключенные и заведомо немые оповещения
отсеиваются при сборке плана — планировать событие, которое ничего не сделает,
незачем.

### 13.2. Цикл событий: отдельная корутина дедлайна вместо гонки

§6.1 предлагает `withTimeoutOrNull(waitMs) { commands.receive() }`. Отменённый
по таймауту `receive()` может потерять уже извлечённый элемент — а потерянная
команда здесь означает не сработавшую кнопку «Пауза» посреди занятия. В
реализации дедлайн живёт отдельной корутиной, которая присылает в тот же канал
`Wake`; цикл всегда читает канал и ничего не теряет. Количество пробуждений то
же самое.

### 13.3. Догон датируется плановой границей этапа

§7 вызывает `completeStage(now, …)`. При этом следующий этап начинал бы отсчёт
с момента пробуждения, и движок навсегда отставал бы ровно на длительность
заморозки — сценарий E-21 («скачок через два этапа») был бы невыполним.
В реализации смена этапа датируется `due.atElapsedMs` — тем моментом, когда
этап должен был начаться. Именно это и делает догон догоном.

Там же исправлена формула дрейфа: `now - nextDeadline(now)` считает по
**будущему** дедлайну (метод отфильтровывает наступившие) и всегда даёт
отрицательное число. Отставание считается от самого раннего **наступившего**
дедлайна.

### 13.4. START этапа, на котором мы оказались, звучит всегда

§7 упоминает это исключение словами; в коде оно выражено явно: при отсеве
просроченных оповещений сохраняется START того этапа, на котором фактически
оказалось занятие. Без него инструктор не поймёт, где он находится, после того
как система вернула приложению управление.

### 13.5. Порядок проверок снимка: возраст важнее часов

§8.3 проверяет расхождение часов до возраста снимка. Часы могли перевести и на
сессии двухчасовой давности, а монотонная метка в этом случае — единственный
честный источник возраста. Реализация проверяет `REBOOTED → TOO_OLD →
CLOCK_CHANGED → OK`.

### 13.6. `TimerEvent.RunStateChanged` и точки персиста

§8.2 требует сохранять сессию на `Start`, `Pause`, `Resume` и `Stop`, но список
событий §9.1 такого сигнала не содержал. Добавлено событие `RunStateChanged`;
завершение занятия по-прежнему приходит отдельным `SessionFinished`.

Персист и перевзвод watchdog переехали из сервиса в `SessionController` и
вызываются **гарантированным обработчиком**, а не через `SharedFlow`: подписчик
на широковещательный поток появляется когда угодно, а сохранить состояние надо
при каждом изменении. Сервису остаётся то, ради чего он существует, —
приоритет процесса, уведомление и звук.

### 13.7. Мелкие уточнения

| Место | Уточнение |
|---|---|
| `Adjust` | клампится сама правка, а не результат: иначе после десяти нажатий «−30 с» первое «+30 с» ничего бы не изменило, и показанная накопленная правка разошлась бы с фактической длительностью |
| `actualDurationsMs` | накапливается, а не перезаписывается: этап, пройденный дважды через «Пред.», занял время оба раза, и общий прогресс обязан остаться монотонным |
| Последний этап | при переходе в FINISHED его время замораживается в `stageElapsedAtResumeMs`, а не переносится в `actualDurationsMs` — инвариант «в `actualDurationsMs` только покинутые этапы» сохраняется, общее время не удваивается |
| `scheduleForCurrentStage` | принимает `now` и считает начало этапа от него назад: формула тогда верна и в PAUSED, где `resumedAtMs` уже неактуален |
| FREE-этап | END-оповещение сохраняется в плане и звучит в момент ручного перехода (решение B-5); отбрасываются только предупреждения |
| `SessionPlan` | пустой план запрещён конструктором — решение B-6 проверяется один раз на границе, а не в каждом методе движка |
