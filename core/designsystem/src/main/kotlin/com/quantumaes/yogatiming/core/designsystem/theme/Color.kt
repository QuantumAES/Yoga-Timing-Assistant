...
/**
 * Палитра рядов графика статистики.
 *
 * Своя, а не `primary` с `tertiary` из схемы, по той же причине, что и у
 * рабочего экрана, только требование другое: два ряда на одном графике обязаны
 * различаться **между собой**. Схема Material этого не гарантирует ни в
 * динамическом варианте (цвета выводятся из обоев), ни в собственном: зелёный
 * `primary` и сине-зелёный `tertiary` приложения расходятся в OKLab всего на
 * ΔE ≈ 9 — при пороге различимости 15 столбики «время» и «занятия» читались бы
 * как один ряд в двух оттенках.
 *
 * Пара проверена расчётом, а не на глаз: зелёный и фиолетовый расходятся на
 * ΔE ≈ 22 при обычном зрении и ≥ 15 при протанопии и дейтеранопии (симуляция
 * Machado — Oliveira — Fernandes), контраст к поверхности карточки ≥ 6:1.
 * Красный и оранжевый как второй ряд не годятся: они в приложении заняты под
 * состояния (ошибка, перерасход), а с зелёным их путает половина видов
 * дальтонизма.
 */
data class ChartPalette(
    /** Общее время практики — главный ряд. */
    val time: Color,
    /** Число занятий — второй ряд. */
    val count: Color,
) {
    companion object {
        val Light =
            ChartPalette(
                time = Color(0xFF1F6B41),
                count = Color(0xFF6750A4),
            )

        val Dark =
            ChartPalette(
                time = Color(0xFF8AD6A0),
                count = Color(0xFFC4A7FF),
            )
    }
}
